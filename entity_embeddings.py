...
def create_model(data, catcols):
    ...
    catcols: list of categorical column names 
    return >> compiled tf.keras model for entity embeddings
    """
    # init list of inputs for embeddings
    inputs = []
    #init list of outputs for embeddigns
    outputs = []
    ...
    for c in catcols:
        ...
        """
        embeding calculator:
        min size: half of the number of unique values in the column 
        max size: 50 (depends on the number of unique categories...)
        """
        ...
        #this becomes our output layer for current features 
...
def run(fold):
    ...
    features = [
    f for f in df.columns if f not in ("id", "incom", "kfold")
    ]
    ...
    # fit the model
    model.fit(xtrain,
            ytrain_cat, validation_data=(xvalid, yvalid_cat), verbose =1, batch_size =1024, epochs =3)
